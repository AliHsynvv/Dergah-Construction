...
export default function PortfolioPage() {
  ...
  const translateCategory = (category: string) => {
    return (t.portfolio.categories as Record<string, string>)[category] || category;
  };

  // Helper function to get project translation
  const getProjectTranslation = (projectTitle: string) => {
    const projectKey = projectTitle as keyof typeof t.portfolio.projects;
    return (t.portfolio.projects[projectKey] || { title: projectTitle, desc: "" });
  };
  ...
  return (
    <main className={`min-h-screen transition-colors duration-500 ${darkMode ? 'bg-slate-900' : 'bg-white'
      ...
      <section className={`relative pt-32 md:pt-36 pb-16 md:pb-20 transition-colors duration-500 ${darkMode
          ...
          <motion.div style={{ y: y1 }}>
            ...
            >
              <motion.div
                ...
                className={`inline-flex items-center rounded-full px-4 py-1.5 mb-4 transition-colors duration-500 ${darkMode
                  ? 'bg-gradient-to-r from-blue-500/20 to-blue-600/15'
                  : 'bg-gradient-to-r from-brand/10 to-blue-600/10'
                  }`}
              >
                <span className={`text-xs font-medium transition-colors duration-500 ${darkMode ? 'text-blue-300' : 'text-brand'
                  }`}>{t.portfolio.badge}</span>
              </motion.div>
              ...
              <p className={`text-sm sm:text-base md:text-lg max-w-2xl mx-auto leading-relaxed px-4 sm:px-0 transition-colors duration-500 ${darkMode ? 'text-slate-300' : 'text-slate-600'
                }`}>
            ...
            >
              {categories.map((category) => (
                <motion.button
                  ...
                  className={`px-3 py-1.5 rounded-full text-xs font-medium transition-all duration-300 ${filter === category
                    ? darkMode
                  ...
                  whileTap={{ scale: 0.95 }}
                >
                  {category === "all" ? t.portfolio.all : translateCategory(category)}
                </motion.button>
              ))}
            </motion.div>
            ...
            >
              {filteredProjects.map((project) => (
                <motion.article
                  key={project.title}
                  variants={item}
                  className={`group relative overflow-hidden rounded-2xl border-2 transition-all duration-500 ${project.featured
                    ? darkMode
                ...
                >
                  <div className="relative h-48 sm:h-52 md:h-56 overflow-hidden">
                    ...
                    <div className="absolute top-4 left-4">
                      <span className={`inline-flex items-center px-3 py-1.5 rounded-full text-xs font-semibold transition-colors duration-500 ${project.featured
                        ? darkMode
                  ...
                  <div className="p-5 md:p-6">
                    <h3 className={`text-lg md:text-xl font-bold tracking-tight mb-2 transition-colors duration-300 ${darkMode
                      ? 'text-slate-100 group-hover:text-blue-400'
                      : 'text-slate-900 group-hover:text-brand'
                      }`}>
                      {getProjectTranslation(project.title).title}
                    </h3>
                    <p className={`text-sm leading-relaxed line-clamp-3 transition-colors duration-500 ${darkMode ? 'text-slate-300' : 'text-slate-600'
                      }`}>
                      {getProjectTranslation(project.title).desc}
                    </p>
